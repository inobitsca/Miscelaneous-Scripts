...
#
#Version 1.2.0 2020-09-09


Import-module Activedirectory
$allUsers = Get-aduser -properties mail, proxyaddresses -Filter 'mail -like "*@*"'
...
$user = 'None'
#Menu Options
$sub1 = 'Change the Primary Address'
$sub2 = 'Add an Alias Address'
$sub3 = 'Remove an Alias address'
$sub4 = 'Check Existing Email Addresess'
$sub5 = ''
$NewEmail =''
$check = ''
...
function Sub1 {
Write-Host "Sub1 function"
$check = ''
$NewEmail = $ChangeEmail.Text
$checkmail = "*" + $NewEmail + "*"
$Check = $allUsers |where {$Samaccountname -ne $SAMName.Text -and $_.proxyaddresses -like $checkmail}

#No Email Address
If (!$NewEmail) 
   {
Write-host "No Email Address entered." -fore Red
...
#Check Existing Proxy
 If ($Check -ne $null) {
 Write-host "Email Address already existsin the organization. Sub1 Function" -fore Red
$NP = "Email Address already exists"
FindUserForm
 }
 #Add new Primary
else{
Set-ADUser -Identity $ID -remove @{Proxyaddresses="SMTP:"+$user.mail}
Set-ADUser -Identity $ID -remove @{Proxyaddresses="smtp:"+$ChangeEmail.text}
Set-ADUser -Identity $ID -Add @{Proxyaddresses="smtp:"+$user.mail}
Set-ADUser -Identity $ID -Add @{Proxyaddresses="SMTP:"+$ChangeEmail.text}
Set-ADUser -Identity $ID -emailaddress $ChangeEmail.text
$allUsers = Get-aduser -properties mail, proxyaddresses -Filter 'mail -like "*@*"'
}
 ####End Code
    }
...
#Add an Alias
function Sub2 {
Write-Host "Sub2 function"
Write-host  "Add alias: " $ChangeEmail.Text
$CheckEmail = $ChangeEmail.Text
$NewEmail = $ChangeEmail.Text
$CheckEmail = "*" + $CheckEmail +"*"
$check = $null
$Check = $allUsers |where {$Samaccountname -ne $SAMName.Text -and $_.proxyaddresses -like $CheckEmail}
Write-host "Address check resut: " $check

if ($check -like "*CN=*") { UsedAddressForm
Write-host "Email Address already existsin the organization." -fore Red
$NP = "Email Address already exists" 
}
...
If (!$Check -and $NewEmail)
    {
Set-ADUser -Identity $ID -Add @{Proxyaddresses="smtp:"+$ChangeEmail.text}
$allUsers = Get-aduser -properties mail, proxyaddresses -Filter 'mail -like "*@*"'
 ####End Code
 [void]$ResultForm.close()
}
...
$allUsers = Get-aduser -properties mail, proxyaddresses -Filter 'mail -like "*@*"'
 ####End Code
 Write-host "Alias $newEmail Removed" -BackgroundColor DarkCyan
 ResultsForm
...
$res = $true
  ...
  ### Test ###
$user = get-aduser $samname.Text -Properties mail,proxyaddresses

# Process form
$ProcessForm                    = New-Object system.Windows.Forms.Form
$ProcessForm.ClientSize         = '500,600'
...
$EmailTitle.text                      = 'Email Addresses for ' + $user.name + " (" +$SamName.Text +  ')'
...
@("Choose an option",$sub1,$sub2,$sub3,$sub4) | ForEach-Object {[void] $OperationChoice.Items.Add($_)}
...
$user = get-aduser $samname.Text -Properties mail,proxyaddresses
If ($res -eq $true){}
if (!$user.name) {write-host "Username invalid:" $SamName.Text -ForegroundColor Red
$Valid = 0
InvalidUserForm 
}

Write-host $OperationChoice.text -fore green
###Sub1
  ### Change the Primary Address ###
if ($OperationChoice.text -eq $sub1) {
# Init PowerShell Gui
Add-Type -AssemblyName System.Windows.Forms
# Result form
$ResultForm                    = New-Object system.Windows.Forms.Form
$ResultForm.ClientSize         = '500,600'
...
$ResultChoice.text                      = $SamName.Text + " ("+ $user.Name +")"
...
$ResultForm.Controls.Add($NoEmailLable)
...
$ResultForm.Controls.Add($ChangeEmailLabel)
...
$ResultForm.Controls.Add($ChangeEmail)
...
$ResultForm.CancelButton   = $cancelBtn3
$ResultForm.Controls.Add($ExecBtn)

$ExecBtn.Add_Click({ Sub1 })
...
$ResultForm.CancelButton   = $cancelBtn3
$ResultForm.Controls.Add($cancelBtn3)

$cancelBtn3.Add_Click({ $ResultForm.Close() })



# Display the form
$result = $ResultForm.ShowDialog()
If ($close = $true) {$ResultForm.Close()}
Write-host $ChangeEmail.text
$NewEmail = $ChangeEmail.Text
$checkmail = "*" + $NewEmail + "*"
$Check = ""
$Check = $allUsers |where {$_.samaccountname -ne $SAMName.Text -and $_.proxyaddresses -like $checkmail} 

If (!$NewEmail) {Write-host "No Address entered. Sub1" -fore Red
$NP = "No email address entered!"
<#if($IT -lt 2){
$IT = $IT + 1
FindUserForm}#>
...
#Check Existing Proxy
 If ($Check -ne $null) {
 Write-host "Email Address already existsin the organization. Sub1" -fore Red
$NP = "Email Address already exists"
<#if($IT -lt 2){
$IT = $IT + 1
FindUserForm}#>
...
#
####Sub2
if ($OperationChoice.text -eq $sub2) {
# Init PowerShell Gui
Add-Type -AssemblyName System.Windows.Forms
# Result form
$ResultForm                    = New-Object system.Windows.Forms.Form
...
#New Email TextBoxLable
$ChangeEmailLabel                = New-Object system.Windows.Forms.Label
$ChangeEmailLabel.text           = "Enter New Alias Email Address:"
$ChangeEmailLabel.AutoSize       = $true
$ChangeEmailLabel.width          = 25
$ChangeEmailLabel.height         = 20
$ChangeEmailLabel.location       = New-Object System.Drawing.Point(20,500)
...
$ExecBtn.Font              = 'Microsoft Sans Serif,10'
$ExecBtn.ForeColor         = "#ffffff"
$ResultForm.CancelButton   = $cancelBtn4
$ResultForm.Controls.Add($ExecBtn)
$ExecBtn.Add_Click({ Sub2 })
...
$ResultForm.CancelButton   = $cancelBtn4
$ResultForm.Controls.Add($cancelBtn4)
$cancelBtn4.Add_Click({ $ResultForm.close() })
...
If (!$NewEmail) {Write-host "No Address entered. Sub2" -fore Red
$NP = "No email address entered!"}
 If ($Check -ne $null) {
 Write-host "Email Address already existsin the organization. Sub1" -fore Red
$NP = "Email Address already exists"}
...
#
####Sub3
if ($OperationChoice.text -eq $sub3) {
# Init PowerShell Gui
Add-Type -AssemblyName System.Windows.Forms
# Result form
$ResultForm                    = New-Object system.Windows.Forms.Form
...
#New Email TextBoxLable
$ChangeEmailLabel                = New-Object system.Windows.Forms.Label
$ChangeEmailLabel.text           = "Select the Alias Email Address to be removed:"
$ChangeEmailLabel.AutoSize       = $true
$ChangeEmailLabel.width          = 25
$ChangeEmailLabel.height         = 20
$ChangeEmailLabel.location       = New-Object System.Drawing.Point(20,500)
...
#New Email TextBox
$ChangeEmail                     = New-Object system.Windows.Forms.ComboBox
$ChangeEmail.text                = "Choose"
$ChangeEmail.width               = 200
$ChangeEmail.autosize            = $true
$ChangeEmail.Visible             = $true  
      
# Add the items in the dropdown list
@($prx1,$prx2,$Prx3,$Prx4,$prx5,$prx6,$prx7,$prx8) | ForEach-Object {if($_) {[void] $ChangeEmail.Items.Add($_)}}
# Select the default value
$ChangeEmail.SelectedIndex       = 0
$ChangeEmail.location            = New-Object System.Drawing.Point(20,525)
$ChangeEmail.Font                = 'Microsoft Sans Serif,10'
$ResultForm.Controls.Add($ChangeEmail)
Write-host $ChangeEmail.text
...
$ExecBtn.Font              = 'Microsoft Sans Serif,10'
$ExecBtn.ForeColor         = "#ffffff"
$ResultForm.CancelButton   = $cancelBtn5
$ResultForm.Controls.Add($ExecBtn)
$ExecBtn.Add_Click({ Sub3 })
...
$ResultForm.CancelButton   = $cancelBtn5
$ResultForm.Controls.Add($cancelBtn5)
$cancelBtn5.Add_Click({ $ResultForm.close() })
...
#
####Sub4
if ($OperationChoice.text -eq $sub4) {
ResultsForm
Write-host "Sub4 Results"
}
#End Sub4

#$Sub5 (Display Results)
if ($OperationChoice.text -eq $sub5) {
ResultsForm
Write-host "Sub5 Results"
}
#### End Sub
}
