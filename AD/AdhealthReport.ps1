...
#       Example: ADHealthReport.ps1 -sendmail
#############################################################################
###########################Define Variables##################################
...
$smtphost = "cas.jse.co.za" 
$from = "cedrica@jse.co.za" 
$email1 = "cedrica@jse.co.za" 
$timeout = "60"
...
Add-Content $report '<title>AD Health Checks</title>' 
...
Write-Host $DC `t $DC `t Ping Success -ForegroundColor Green
		...
		$serviceStatus = start-job -scriptblock {get-service -ComputerName $($args[0]) -Name "Netlogon" -ErrorAction SilentlyContinue} -ArgumentList $DC
                ...
                {
                 Write-Host $DC `t Netlogon Service TimeOut -ForegroundColor Red
 		   ...
 		   Write-Host $DC `t $serviceStatus1.name `t $serviceStatus1.status -ForegroundColor Green 
         	   ...
         	   Add-Content $report "<td bgcolor= 'Aquamarine' align=center><B>$svcState</B></td>" 
                  ...
                  { 
       		  Write-Host $DC `t $serviceStatus1.name `t $serviceStatus1.status -ForegroundColor Yellow 
         	  $svcName = $serviceStatus1.name 
         	  $svcState = $serviceStatus1.status          
		...
		$serviceStatus = start-job -scriptblock {get-service -ComputerName $($args[0]) -Name "NTDS" -ErrorAction SilentlyContinue} -ArgumentList $DC
                ...
                if($serviceStatus.state -like "Running")
                {
                 Write-Host $DC `t NTDS Service TimeOut -ForegroundColor Red
                 Add-Content $report "<td bgcolor= 'Red' align=center><B>NTDSTimeout</B></td>"
                 stop-job $serviceStatus
                }
                else
                {
 		   ...
 		   Write-Host $DC `t $serviceStatus1.name `t $serviceStatus1.status -ForegroundColor Green 
         	  ...
         	  $svcName = $serviceStatus1.name 
         	  $svcState = $serviceStatus1.status          
         	  Add-Content $report "<td bgcolor= 'Yellow' align=center><B>$svcState</B></td>" 
                  } 
                }
               ######################################################
                ##############DNS Service Status################
		$serviceStatus = start-job -scriptblock {get-service -ComputerName $($args[0]) -Name "DNS" -ErrorAction SilentlyContinue} -ArgumentList $DC
                ...
                if($serviceStatus.state -like "Running")
                {
                 Write-Host $DC `t DNS Server Service TimeOut -ForegroundColor Red
                 Add-Content $report "<td bgcolor= 'Red' align=center><B>DNSTimeout</B></td>"
                 stop-job $serviceStatus
                }
                else
                {
 		   ...
 		   Write-Host $DC `t $serviceStatus1.name `t $serviceStatus1.status -ForegroundColor Green 
               ...
               {
               Write-Host $DC `t Netlogons Test TimeOut -ForegroundColor Red
               Add-Content $report "<td bgcolor= 'Red' align=center><B>NetlogonsTimeout</B></td>"
               stop-job $sysvol
               }
               else
               {
               ...
               if($cmp::instr($sysvol1, "passed test NetLogons"))
                  ...
                  {
                  Write-Host $DC `t Netlogons Test Failed -ForegroundColor Yellow
                  Add-Content $report "<td bgcolor= 'Yellow' align=center><B>NetlogonsFail</B></td>"
                  }
                }
               ########################################################
               ####################Replications status##################
               ...
               if($sysvol.state -like "Running")
               {
               Write-Host $DC `t Replications Test TimeOut -ForegroundColor Red
               Add-Content $report "<td bgcolor= 'Red' align=center><B>ReplicationsTimeout</B></td>"
               stop-job $sysvol
               }
               else
               {
               ...
               if($cmp::instr($sysvol1, "passed test Replications"))
                  ...
                  {
                  Write-Host $DC `t Replications Test Failed -ForegroundColor Yellow
                  Add-Content $report "<td bgcolor= 'Yellow' align=center><B>ReplicationsFail</B></td>"
                  }
                }
               ########################################################
	       ####################Services status##################
               ...
               if($sysvol.state -like "Running")
               {
               Write-Host $DC `t Services Test TimeOut -ForegroundColor Red
               Add-Content $report "<td bgcolor= 'Red' align=center><B>ServicesTimeout</B></td>"
               stop-job $sysvol
               }
               else
               {
               ...
               if($cmp::instr($sysvol1, "passed test Services"))
                  ...
                  {
                  Write-Host $DC `t Services Test Failed -ForegroundColor Yellow
                  Add-Content $report "<td bgcolor= 'Yellow' align=center><B>ServicesFail</B></td>"
                  }
                }
               ########################################################
	       ####################Advertising status##################
               ...
               if($sysvol.state -like "Running")
               {
               Write-Host $DC `t Advertising Test TimeOut -ForegroundColor Red
               Add-Content $report "<td bgcolor= 'Red' align=center><B>AdvertisingTimeout</B></td>"
               stop-job $sysvol
               }
               else
               {
               ...
               if($cmp::instr($sysvol1, "passed test Advertising"))
                  ...
                  {
                  Write-Host $DC `t Advertising Test Failed -ForegroundColor Yellow
                  Add-Content $report "<td bgcolor= 'Yellow' align=center><B>AdvertisingFail</B></td>"
                  }
                }
               ########################################################
	       ####################FSMOCheck status##################
               ...
               if($sysvol.state -like "Running")
               {
               Write-Host $DC `t FSMOCheck Test TimeOut -ForegroundColor Red
               Add-Content $report "<td bgcolor= 'Red' align=center><B>FSMOCheckTimeout</B></td>"
               stop-job $sysvol
               }
               else
               {
               ...
               if($cmp::instr($sysvol1, "passed test FsmoCheck"))
                  ...
                  {
                  Write-Host $DC `t FSMOCheck Test Failed -ForegroundColor Yellow
                  Add-Content $report "<td bgcolor= 'Yellow' align=center><B>FSMOCheckFail</B></td>"
                  }
                }
               ########################################################
