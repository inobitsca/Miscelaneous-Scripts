...
#Convert SAMAccoutName to a String (Needed for AD Filter, it only takes string inputs)
$EMPIDString = $ImportEMPID.SAMACCOUNTNAME.ToString()

#Search AD for Employee based on SamAccontName
$EMPUser = Get-ADUser -Filter {SAMACCOUNTNAME -eq $EMPUser} -properties *
...
Set-ADAccountControl -Identity $EMPUser.SamAccountName -Enabled -ErrorAction SilentlyContinue $false

#Move User into created Disabled OU based on date
Get-aduser $EMPUser.SamAccountName | Move-ADObject -TargetPath "OU=$OUName,OU=Disabled to be Deleted,DC=mccarthyltd,DC=local" 
}
