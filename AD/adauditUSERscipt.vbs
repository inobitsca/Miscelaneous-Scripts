...
' Description       Rips out all users and their context from AD and writes them to a csv file
'
'
...
objCommand.CommandText = _
   "<LDAP://" & LDAPADsPath &  ">;(objectCategory=user);sAMAccountName,distinguishedName,name,whenCreated,homeDirectory,scriptPath,displayName,mail,msExchHomeServerName;subtree"
...
'
''''' Do an LDAP call and get all users in AD and save the users data
'
While Not objRecordSet.EOF
    ...
    Call Stripper                                   ' Build table of the context of the user

    cntX = cntX + 1
    cntY = 0

    objRecordSet.MoveNext			     ' Move to next user
Wend
...
FileDate = FileLocation & YearDate & MonthDate & DayDate & "_Useraccts.csv"
...
'
''''' Finally output the data (This system is only setup for a max of 10,000 users
'
For cntL = 0 to 9999                                ' Cycle through all users found
   ...
   If exFlg = 0 Then
      ...
      ts.write(aryCN(cntL,0)) & ", "                   ' Output users logon name

      'Drop all comma's in the context
      tmpName = Replace(arySvCN(cntL), ",", " ")
      ts.write(tmpName) & ", "                         ' Output full context of user
