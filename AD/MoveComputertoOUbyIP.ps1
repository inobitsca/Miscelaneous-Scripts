...
$old = (Get-Date).AddDays(-180) # Modify the -60 to match your threshold 

#Set the threshold for an "very old" computer which will be deleted
$veryold = (Get-Date).AddDays(-360) # Modify the -180 to match your threshold 
...
# Disabled OU
$DisabledDN = "OU=NoActivity180Days,OU=ComputersToMove,DC=mccarthyltd,DC=local"
...
Get-ADComputer -Filter { OperatingSystem -like "Windows *" } -SearchScope Subtree -SearchBase "CN=Computers,DC=McCarthyLtd,DC=Local"  | ForEach-Object {
	...
	# If the computer is more than 360 days off the network, remove the computer object
...
$DestinationDN = $VWMastercarsDN
	...
	Else {
		...
		$DestinationDN = "OU=NoSubnetFound,OU=ComputersToMove,DC=mccarthyltd,DC=local"	
	}
...
if ($IP -eq $NULL) 
{
		Move-ADObject -Identity $ComputerDN -TargetPath "OU=NoSubnetFound,OU=ComputersToMove,DC=mccarthyltd,DC=local"
	}}