...
{
	New-ADUser $_.Username -Path "OU=Test Users,$ForestDn" -OtherAttributes @{givenName="$($_.FirstName)";sn="$($_.LastName)";displayName="$($_.FirstName) $($_.LastName)";title="$($_.Title)"} -ErrorAction SilentlyContinue
}